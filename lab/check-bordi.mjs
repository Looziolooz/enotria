import { chromium } from 'playwright-core';
import http from 'http'; import fs from 'fs'; import path from 'path';
const DIST=path.resolve('dist');
const MIME={'.html':'text/html','.js':'application/javascript','.css':'text/css','.webp':'image/webp','.json':'application/json','.woff2':'font/woff2','.png':'image/png'};
const srv=await new Promise(r=>{const s=http.createServer((q,res)=>{let u=decodeURIComponent(q.url.split('?')[0]);if(u==='/')u='/index.html';const f=path.join(DIST,u);if(fs.existsSync(f)&&fs.statSync(f).isFile()){res.setHeader('Content-Type',MIME[path.extname(f)]||'application/octet-stream');fs.createReadStream(f).pipe(res);}else{res.statusCode=404;res.end();}});s.listen(4428,()=>r(s));});
const b=await chromium.launch({executablePath:'C:/Program Files (x86)/Google/Chrome/Application/chrome.exe',headless:true});
const p=await b.newPage({viewport:{width:1440,height:900}});
const err=[]; p.on('pageerror',e=>err.push(e.message));
await p.goto('http://localhost:4428/',{waitUntil:'networkidle'});
await p.waitForTimeout(3500);
const H=await p.evaluate(()=>{const a=document.querySelector('[data-act="produttori"]');return Math.round(a.getBoundingClientRect().top+scrollY);});
let migliore=null;
for(const at of [0.055,0.150,0.360,0.545,0.730]){
  await p.evaluate(v=>window.scrollTo(0,v), Math.round(at*H));
  await p.waitForTimeout(1300);
  const r=await p.evaluate(()=>{
    const vis=[...document.querySelectorAll('.inserto')].filter(n=>+n.style.opacity>0.3);
    const c=document.querySelector('.inserti');
    return {n:vis.length, op:vis.map(n=>(+n.style.opacity).toFixed(2)).join(','),
      tono:c&&c.classList.contains('inserti--inchiostro')?'inchiostro':'calce'};
  });
  console.log(`  ${(at*100).toFixed(1)}%  ${r.n} segno/i (${r.op||'-'}) · ${r.tono}`);
  if(r.n && !migliore) migliore=at;
}
if(migliore){ await p.evaluate(v=>window.scrollTo(0,v), Math.round(migliore*H)); await p.waitForTimeout(1200);
  await p.screenshot({path:'lab/shots/bordi.png', clip:{x:0,y:0,width:1440,height:900}}); }
console.log('pageerror:', err.length);
await b.close(); srv.close();

...
await p.goto('http://localhost:4414/',{waitUntil:'networkidle'});
await p.waitForTimeout(2200);
const stageH=47700;
for(const [nome,at] of [['capitolo-pietra',0.121],['capitolo-botte',0.605]]){
  await p.evaluate(v=>window.scrollTo(0,v), Math.round(at*stageH));
  await p.waitForTimeout(4500);
  await p.screenshot({path:`lab/shots/${nome}.png`, clip:{x:0,y:0,width:1440,height:900}});
}

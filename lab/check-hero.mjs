...
await p.goto('http://localhost:4415/',{waitUntil:'networkidle'});
await p.waitForTimeout(4500);
await p.evaluate(()=>window.scrollTo(0, 500));
await p.waitForTimeout(1500);
await p.screenshot({path:'lab/shots/hero.png', clip:{x:0,y:0,width:1440,height:900}});
const r=await p.evaluate(()=>({
  hud:!!document.querySelector('.hud'),
  hudTesto:(document.querySelector('.hud__testo')||{}).textContent||'',
  heroVisibile:(()=>{const h=document.querySelector('.st-hero'); if(!h) return false;
    return +getComputedStyle(h.querySelector('.stage-text__inner')).opacity;})(),
}));
